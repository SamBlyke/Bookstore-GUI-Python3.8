...
win=Tk(screenName="BookStore")
win.wm_title("BookStore")
def view_command():
    lb1.delete(0,END)
    rows=database1.viewall()
    for row in rows:
        lb1.insert(END,row)

def search_command():
    lb1.delete(0,END)
    rows=database1.search(title_text.get(),author_text.get(),year_text.get(),isbn_text.get())
    for row in rows:
        lb1.insert(END,row)

def add_command():
    lb1.delete(0,END)
    database1.insert(title_text.get(),author_text.get(),year_text.get(),isbn_text.get())
    lb1.insert(END,(title_text.get(),author_text.get(),year_text.get(),isbn_text.get()))

def delete_command():
    lb1.delete(0,END)
    database1.remove(value[0])
    rows=database1.viewall()
    for row in rows:
        lb1.insert(END,row)

def update_command():
    database1.update(title_text.get(),author_text.get(),year_text.get(),isbn_text.get(),value[0])
    lb1.delete(0,END)
...
lb1=Listbox(win,height=6,width=46)
